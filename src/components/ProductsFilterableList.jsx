...
export function ProductsFilterableList({ products = [], selectedFamily, onClearFamilyFilter }) {
  ...
  if (selectedFamily) {
    shown = products.filter(
      (p) =>
        p.owner_id === selectedFamily.id ||
        p.profiles?.family_name === selectedFamily.family_name
    );
  ...
  return (
    <section id="products" className="section source-section">
      <div className="section-title">
        <div>
          ...
          <p style={{ color: '#57534e', fontSize: '15px' }}>
            استكشفي أحدث إبداعات الأسر المنتجة المسجلة بالمنصة.
          </p>
        </div>
      </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontWeight: '700', color: '#ffffff', fontSize: '15px' }}>
              ...
              <strong style={{ color: '#fbbf24' }}>
                {selectedFamily.family_name || 'الأسرة'}
              </strong>{' '}
              ({shown.length} منتجات)
            </span>
          </div>
      ...
      {/* Grid with pre-rendered server products rendered into client container */}
      <div className="grid">
        {shown.map((product) => (
